...
public class Product {
    ...
    public long stock;
    public double price;
    public byte[] pictureAsByteArray;
    public String supplierContact;

    public Bitmap getPicture() {
        if (pictureAsByteArray == null) {
            return null;
        }
        return BitmapFactory.decodeByteArray(pictureAsByteArray, 0, pictureAsByteArray.length);
    }
    ...
    public ContentValues getContentValues() {
        ...
        values.put(COLUMN_PICTURE, pictureAsByteArray);
    ...
    public static Product fromCursor(Cursor cursor) {
        ...
        if (columnIndex != -1 && !cursor.isNull(columnIndex)) {
            product.pictureAsByteArray = cursor.getBlob(columnIndex);
        }
    ...
    public void sale(Context context) throws NotEnoughInStockToSellException {
        if (this.stock == 0) {
            throw new NotEnoughInStockToSellException();
        }
        this.stock -= 1;
        DatabaseManager.getInstance(context).updateProduct(this);
    }
    ...
    public class NotEnoughInStockToSellException extends Throwable {
    }
}
